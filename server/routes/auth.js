...
router.get('/user/active/list',
    auth.isAuth,
    user.getuserActiveList
)

router.get('/user/nonActive/list',
    auth.isAuth,
    user.getusernonActiveList
)
...
router.post('/verify',
    ...
    (request, response, next) => {
        //next()
        return apiResponse.successResponseWithData(response, `Registration Successfull`, { token: request.body.token, _id: request.body.authUser._id, modules: request.body.assignedModules })
    },
...
router.post('/send/otp',
    ...
    user.updateOTP,
    (request, response, next) => {
        return apiResponse.successResponseWithData(response, "OTP Sent Successfully", { otp: request.body.otp })
    }
)
...
router.post('/userId/verify',
    ...
    user.isUserValid,
    (request,response)=>{
        return apiResponse.successResponseWithData(response, `Verification Successfull`, { user:request.body.user})
    }
)
