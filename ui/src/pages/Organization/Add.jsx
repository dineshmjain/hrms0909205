...
import {
  ...
  OrganizationDefaultCreateAction,
  OrganizationGroupCreateAction,
} from "../../redux/Action/Oraganization/OrganizationAction";
...
console.log("Redux state:", typeOfIndustryList);
  ...
  useEffect(() => {
    dispatch(GetTypeOfIndustry()).then(({ payload }) => {
      if (payload?.status == 200) {
        setForm({ ...form, orgTypeId: payload?.data?.[0]?._id });
      }
    });
  }, [dispatch]);
  ...
  const validate = () => {
    ...
    if (!form.groupName) {
      ...
      temp.orgName = "Organization name is required";
    } else if (form.orgName.length < 3) {
      temp.orgName = "Organization name must be at least 3 characters";
    }
    if (!form.orgTypeId) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { structure, orgName, orgTypeId, groupName } = form;
      const cleanedAddress = finalData?.address || {};
      const formData = {};
      if (structure == "group" && groupName == "") {
      ...
      if (structure === "branch") {
        ...
        Object.assign(formData, form); // fallback
      }
      ...
      let result;
      if (structure === "branch") {
        result = await dispatch(OrganizationDefaultCreateAction(dataToSend));
      } else if (structure === "organization") {
        result = await dispatch(OrganizationCreateAction(dataToSend)); // added dispatch for consistency
      } else {
        result = await dispatch(OrganizationGroupCreateAction(dataToSend));
      }
      ...
      if (meta?.requestStatus === "fulfilled") {
        ...
        toast.success(
          payload?.message || `You've successfully created your account`
        );
        navigate("/dashboard");
        ...
        const backendMessage =
          errData?.validation?.body?.message ||
          errData?.message ||
          "Submission failed";

        toast.error(backendMessage);

        console.warn("Backend validation failed:", errData?.validation?.body);
      }
    } catch (error) {
  ...
  console.log(form.structure, "erererererer");

  return (
    <div className="flex w-full flex-col min-h-[70vh] p-8 gap-8">
      ...
      <Card className="p-8 shadow-md border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <SingleSelectDropdown
                    ...
                    selectedOption={form?.orgTypeId}
                    hideLabel={true}
                    handleClick={(value) => {
                      handleChange({
                        target: { name: "orgTypeId", value: value?._id },
                      });
                    }}
                  ></SingleSelectDropdown>
                  {/* <Select
          ...
          <div className="flex justify-end gap-4 pt-4">
            <Button type="button" onClick={handleNext} className="bg-primary">
              {showOrg ? "Next" : "Prev"}
            </Button>
            {!showOrg && (
