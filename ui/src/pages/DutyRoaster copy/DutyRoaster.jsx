...
const isIntersecting = (r1, r2) =>
  ...
  // Handle fetching shifts with debounce
  useEffect(() => {
    ...
    setFiltersData((prev) => {
      ...
      if (filterType === 'clientOrg') {
        delete updated.orgIds;
        delete updated.branchIds;
      } else {
        delete updated.clientMappedIds;
        delete updated.clientBranchIds;
      }
      return updated;
    });
  ...
  // ✅ fetch shifts on explicit trigger only
  useEffect(() => {
  ...
  // ✅ optional: fetch once on first render
  useEffect(() => {
    setShouldFetch(true);
  }, []);
...
// Reset filter data and page when filterType changes
useEffect(() => {
  ...
  const doTimeRangesOverlap = (startA, endA, startB, endB) => {
    ...
    const cleanedParams = removeEmptyStrings(baseParams);
    ...
    const hasValidFilters =
      (filterType === 'myOrg' && (cleanedParams?.orgIds || cleanedParams?.branchIds)) ||
      (filterType === 'clientOrg' && (cleanedParams?.clientMappedIds || cleanedParams?.clientBranchIds));

    if (hasValidFilters) {
      console.log('Dispatching shift list with:', cleanedParams);
      dispatch(ShiftListbyDateAction(cleanedParams));
    } else {
      console.warn('Skipping Shift fetch — no valid filters.');
    }
  };
