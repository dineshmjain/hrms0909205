...
import { LeavePolicyUpdateAction, LeavePolicyNameGetAction } from "../../../redux/Action/Leave/LeaveAction";
  ...
  useEffect(() => {
    if (state && branchList.length > 0) {
      ...
      if (state.carryForwardEnabled) {
        ...
        handlingType = {
          label: state.salaryConversionCycle === "yearly" ? "Yearly" : "Monthly",
      ...
      setForm({
        ...
        yearlyCount: state.noOfDays || "",
        isMonthly: state.cycle?.type === "monthly",
        monthlyCount: state.cycle?.type === "monthly" ? state.noOfDays : "",
