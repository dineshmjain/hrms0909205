...
const BranchRadiusSetting = () => {
  ...
  }, [filterType,subOrg, selectedClient]);
      ...
      if (checkModules("suborganization", "r")) {
        dispatch(SubOrgListAction({})).then(({ payload }) => {
          if (payload?.data?.length) {
            setSubOrg(payload.data[payload.data.length - 1]?._id);
          }
        });
      }
    } else {
  ...
  const editButton = (branch) => {
    ...
    if(filterType==="myOrg")
    {
    navigate("/branchradiussetting/add", { state: branch, subOrgId: subOrg, });
    }
    else{
      console.log("Editing branch in client organization:", { state: { ...branch, isClient: true, subOrgId: "", clientMappedId: selectedClient?._id } });
      navigate("/branchradiussetting/add", { state: { ...branch, isClient: true, subOrgId: "", clientMappedId: selectedClient?._id } });
    }
  };
  ...
  const labels = {
    ...
    firstName: { DisplayName: "Created By", type: "object", objectName: "createdBy" },
  ...
  return (
    <div className="flex flex-col gap-4 p-2 w-full h-full">
      ...
      <Header
        ...
        buttonTitle="Add"
        headerLabel="Geo Fencing"
        subHeaderLabel="Manage Geo Fencing Settings"
        isButton={false}
      />
      {/* Filters */}
        ...
        <div className="inline-flex rounded-md overflow-hidden shadow my-2 border border-gray-200">
          {["myOrg", "clientOrg"].map((type) => (
            <div
              ...
              className={`px-4 py-2 font-medium text-[12px] transition-all duration-150 ${filterType === type
