...
const BranchManagement = () => {
  ...
  // Load SubOrg and Branch lists on mount
  useEffect(() => {
  ...
  // Stable getPriceList function with proper dependency management
  ...
  // Handle filter changes (date, SubOrg, Branch, Designation)
  useEffect(() => {
  ...
  // Handle SubOrg selection change
  ...
  const processedData = useMemo(() => {
        ...
        if (history && Array.isArray(history)) {
          history.forEach((historyEntry) => {
            data.push({
              ...
              priceData: historyEntry.v?.priceData || historyEntry.v,
    ...
    data = data.filter(item => {
      ...
      if (priceRangeFilter.min || priceRangeFilter.max) {
        const daily = getMinMaxPrice(item.priceData?.daily)
        if (priceRangeFilter.min && daily.min < parseInt(priceRangeFilter.min)) return false
        if (priceRangeFilter.max && daily.max > parseInt(priceRangeFilter.max)) return false
      }
  ...
  const exportData = () => {
    const csvContent = [
      ...
      ...processedData.map(item => {
        const daily = getMinMaxPrice(item.priceData?.daily)
        const monthly = getMinMaxPrice(item.priceData?.monthly)
        const yearly = getMinMaxPrice(item.priceData?.yearly)
        return [
  ...
  // Date input handler to prevent unnecessary updates
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 rounded-md text-sm font-medium text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 transition-colors"
            >
              <TrendingUp className="h-4 w-4 inline mr-2" />
              Base Pricing
            </button>
            <button
              className="px-4 py-2 rounded-md text-sm font-medium bg-blue-600 text-white shadow-sm"
            >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
          <div className="flex flex-col space-y-4">
            ...
            {/* Enhanced filters grid with SubOrg and Branch */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
              {/* SubOrg Filter */}
             {checkModules("suborganization", "r")  &&<div>
