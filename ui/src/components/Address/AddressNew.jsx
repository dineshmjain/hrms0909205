...
import { getCurrentLocation, getLocationWithLatLongFull } from "../../constants/reusableFun";
...
const AddressNew = ({
  ...
  const getZoomFromRadius = (r) => {
    ...
    return Math.min(Math.max(Math.floor(17 - Math.log2(radiusVal / 100)), 10), 21);
  };

  /** ✅ Initialize Google Autocomplete */
  useEffect(() => {
    if (window.google?.maps && !autocompleteService) {
  ...
  useEffect(() => {
    ...
    ["city", "state", "country", "pincode", "district", "geoAddress"].forEach((f) =>
      setFieldValue(getFieldName(f), location[f] || "")
    );
  }, [location]);
...
useEffect(() => {
  if (!location && structuredAddress?.geoJson?.coordinates?.length === 2) {
    ...
    if (!isNaN(lat) && !isNaN(lng)) {
      ...
      setFieldValue(getFieldName("mapCenter"), { lat: Number(lat), lng: Number(lng) });
      setFieldValue(getFieldName("geoAddress"), structuredAddress?.geoLocation?.address || "");
    }
  }
}, [structuredAddress]);
...
console.log(values,'suggestionssuggestions')
    ...
    placesService.getDetails({ placeId }, (place, status) => {
      if (status === window.google.maps.places.PlacesServiceStatus.OK && place.geometry?.location) {
        ...
        const locData = {
          ...
          district: extractAddressComponent(components, "administrative_area_level_2"),
  ...
  useEffect(() => {
    if (mapRef.current && location?.lat && location?.lng) {
      mapRef.current.panTo({ lat: Number(location.lat), lng: Number(location.lng) });
    }
  }, [location]);
    ...
    <div className="w-full p-2">
      ...
      {isMap && geoAddress && (
        <div>
          ...
          <Typography className="text-gray-700 text-[14px] mb-1 font-semibold">{geoAddress}</Typography>
        </div>
      )}
      ...
      {isCitySearchWithTimezone && (
        isEditAvaliable ? (
          <div>
            <Typography className="text-gray-700 text-[14px] font-medium">GPS Location</Typography>
            <Typography className="text-gray-700 text-[14px] mb-1 font-semibold">{geoAddress}</Typography>
          </div>
        ) : (
          <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
        ...
        {["hno", "street", "landmark", "city", "district", "taluk", "state", "country", "pincode"].map(
          (field) => (
            <FormikInput
              key={field}
        ...
        <div className="h-[400px] rounded-md overflow-hidden mt-4 bg-white">
          {!isEditAvaliable && (
            <div className="relative">
              <Typography className="text-gray-800 text-[14px] mb-1 font-medium">GPS Location</Typography>
              <Input
          ...
          <GoogleMap
            ...
            center={{ lat: Number(mapCenter?.lat || 0), lng: Number(mapCenter?.lng || 0) }}
